...
# genesis block is defined using the "ThreeOrgsApplicationGenesis" profile at the bottom
# of the file. This profile defines an application channel consisting of our three Peer Orgs.
...
function networkUp() {
  ...
  if [ ! -d "organizations/peerOrganizations" ]; then
    createOrgs
    infoln "Organizations creations ok"
  fi
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    #${CONTAINER_CLI} volume rm docker_orderer.irischain.com docker_peer0.org1.irischain.com docker_peer0.org2.irischain.com docker_peer0.org3.irischain.com
...
if [ "$MODE" == "up" ]; then
  ...
  networkUp
elif [ "$MODE" == "createOrga" ]; then
  infoln "Creating Organization"
  createOrgs
